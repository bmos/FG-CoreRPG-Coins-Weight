...
local function createCoinsItem(nodeChar)
    if nodeChar.getParent().getName() == 'charsheet' then
        ...
        if nodeFirstInventory then
            ...
            DB.setValue(nodeCoinsItem, 'name', 'string',
                        COINS_INVENTORY_ITEM_NAME)
            DB.setValue(nodeCoinsItem, 'count', 'number', 1)
            DB.setValue(nodeCoinsItem, 'type', 'string', 'Wealth and Money')
            DB.setValue(nodeCoinsItem, 'description', 'formattedtext', '<p>' ..
                            Interface.getString("item_description_coins") ..
                            '</p>')
...
local function findCoinsItem(nodeChar)
    ...
    for _, sItemList in pairs(tItemLists) do
        for _, nodeItem in pairs(DB.getChildren(nodeChar, sItemList)) do
            ...
            if sItemName == COINS_INVENTORY_ITEM_NAME or
                string.match(sItemName:lower(),
                             '^%W*coins%W+coins%W+weight%W+extension%W*$') then
                return nodeItem
            end
        end
    end
    ...
    if nodeCoinsItem then
        ...
        if (nTotalCoinsWeight <= 0 and nTotalCoinsWealth == 0) then
            ...
            if nodeCoinsItemShortcut then
                nodeCoinsItemShortcut.delete()
            end
        elseif nTotalCoinsWeight < 0 then
            DB.setValue(nodeCoinsItem, 'cost', 'string',
                        nTotalCoinsWealth .. ' gp')
            DB.setValue(nodeCoinsItem, 'weight', 'number', 0) -- coins can't be negative weight
            DB.setValue(nodeCoinsItem, 'count', 'number', 1)
            DB.setValue(nodeChar, 'coinitemshortcut', 'windowreference', 'item',
                        nodeCoinsItem.getNodeName());
        else
            DB.setValue(nodeCoinsItem, 'cost', 'string',
                        nTotalCoinsWealth .. ' gp')
            DB.setValue(nodeCoinsItem, 'weight', 'number', round(
                            nTotalCoinsWeight,
                            determineRounding(nTotalCoinsWeight)))
            DB.setValue(nodeCoinsItem, 'count', 'number', 1)
            DB.setValue(nodeChar, 'coinitemshortcut', 'windowreference', 'item',
                        nodeCoinsItem.getNodeName());
        end
...
local function computeCoins(nodeChar)
    ...
    for _, sCurrencyPath in pairs(tCurrencyPaths) do
        for _, nodeCoinSlot in pairs(DB.getChildren(nodeChar, sCurrencyPath)) do
            ...
            local sDenomination = string.lower(
                                      DB.getValue(nodeCoinSlot, 'name', ''))
            local tCurrency = CurrencyManager.getCurrencyRecord(sDenomination)
            if tCurrency then
                nTotalCoinsWealth = nTotalCoinsWealth +
                                        (nCoinAmount *
                                            (tCurrency['nValue'] or 0))
                nTotalCoinsWeight = nTotalCoinsWeight +
                                        (nCoinAmount *
                                            (tCurrency['nWeight'] or .02))
            else
...
function onInit()
    CharEncumbranceManager.calcDefaultCurrencyEncumbrance =
        calcDefaultCurrencyEncumbrance_new
    if Session.IsHost then
        local tCurrencyPaths = CurrencyManager.getCurrencyPaths("charsheet");
        for _, sCurrencyPath in pairs(tCurrencyPaths) do
            DB.addHandler("charsheet.*." .. sCurrencyPath .. '.*',
                          'onChildUpdate', onCoinsValueChanged)
            DB.addHandler("charsheet.*." .. sCurrencyPath, 'onChildDeleted',
                          onCoinsDeleted)
        end
        DB.addHandler(CurrencyManager.CAMPAIGN_CURRENCY_LIST .. '.*.',
                      'onChildUpdate', onDenominationsChanged)
        DB.addHandler(CurrencyManager.CAMPAIGN_CURRENCY_LIST, 'onChildDeleted',
                      onDenominationsChanged)
    end
end
